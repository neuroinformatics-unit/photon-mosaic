...
@pytest.fixture
def custom_metadata_env(tmp_path, metadata_base_config, test_data_root):
    """Set up test environment for custom metadata format."""
    raw_data = tmp_path / "raw_data"
    custom_data_path = test_data_root / "data_custom_metadata"

    if not custom_data_path.exists():
        pytest.skip(
            f"Custom metadata test data not found at {custom_data_path}"
        )

    shutil.copytree(custom_data_path, raw_data)
...
@pytest.fixture
def neuroblueprint_env(tmp_path, metadata_base_config, test_data_root):
    """Set up test environment for NeuroBlueprint metadata format."""
    raw_data = tmp_path / "raw_data"
    nb_data_path = test_data_root / "data_neuroblueprint"

    if not nb_data_path.exists():
        pytest.skip(f"NeuroBlueprint test data not found at {nb_data_path}")

    shutil.copytree(nb_data_path, raw_data)
